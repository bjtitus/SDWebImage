...
@implementation TMSDImageCachesManagerOperation {
    TMSD_LOCK_DECLARE(_pendingCountLock);
}
...
- (instancetype)init {
    if (self = [super init]) {
        TMSD_LOCK_INIT(_pendingCountLock);
        _pendingCount = 0;
    }
    return self;
}
...
- (NSUInteger)pendingCount {
    TMSD_LOCK(_pendingCountLock);
    NSUInteger pendingCount = _pendingCount;
    TMSD_UNLOCK(_pendingCountLock);
    return pendingCount;
}
