...
@implementation TMSDDisplayLink
...
#elif TMSD_IOS || TMSD_TV
        TMSDWeakProxy *weakProxy = [TMSDWeakProxy proxyWithTarget:self];
...
+ (instancetype)displayLinkWithTarget:(id)target selector:(SEL)sel {
    TMSDDisplayLink *displayLink = [[TMSDDisplayLink alloc] initWithTarget:target selector:sel];
    return displayLink;
}
...
static CVReturn DisplayLinkCallback(CVDisplayLinkRef displayLink, const CVTimeStamp *inNow, const CVTimeStamp *inOutputTime, CVOptionFlags flagsIn, CVOptionFlags *flagsOut, void *displayLinkContext) {
    ...
    TMSDDisplayLink *object = (__bridge TMSDDisplayLink *)displayLinkContext;
    if (inOutputTime) {
        object.outputTime = *inOutputTime;
    }
    __weak TMSDDisplayLink *weakObject = object;
