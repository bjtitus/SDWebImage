 ...
 */
@interface TMSDImageCacheConfig : NSObject <NSCopying>

/**
 Gets the default cache config used for shared instance or initialization when it does not provide any cache config. Such as `TMSDImageCache.sharedImageCache`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TMSDImageCacheConfig *defaultCacheConfig;
...
/*
 * The option to control weak memory cache for images. When enable, `TMSDImageCache`'s memory cache will use a weak maptable to store the image at the same time when it stored to memory, and get removed at the same time.
 ...
 * When enabling this option, we will sync back the image from weak maptable to strong cache during next time top level `tmsd_setImage` function call.
 ...
 */
@property (assign, nonatomic) TMSDImageCacheConfigExpireType diskCacheExpireType;
...
/**
 * The custom memory cache class. Provided class instance must conform to `TMSDMemoryCache` protocol to allow usage.
 * Defaults to built-in `TMSDMemoryCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
@property (assign, nonatomic, nonnull) Class memoryCacheClass;

/**
 * The custom disk cache class. Provided class instance must conform to `TMSDDiskCache` protocol to allow usage.
 * Defaults to built-in `TMSDDiskCache` class.
 * @note This value does not support dynamic changes. Which means further modification on this value after cache initialized has no effect.
 */
