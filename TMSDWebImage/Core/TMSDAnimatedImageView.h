...
/**
 ...
 Call `setImage:` with `UIImage(NSImage)` which conforms to `TMSDAnimatedImage` protocol will start animated image rendering. Call with normal UIImage(NSImage) will back to normal UIImageView(NSImageView) rendering
 ...
 */
@interface TMSDAnimatedImageView : UIImageView
/**
 ...
 @warning Pay attention if you directly update the player's property like `totalFrameCount`, `totalLoopCount`, the same property on `TMSDAnimatedImageView` may not get synced.
 */
@property (nonatomic, strong, readonly, nullable) TMSDAnimatedImagePlayer *player;
...
/// Asynchronous setup animation playback mode. Default mode is TMSDAnimatedImagePlaybackModeNormal.
@property (nonatomic, assign) TMSDAnimatedImagePlaybackMode playbackMode;
...
/**
 ...
 @note The best practice to use incremental load is using `initWithAnimatedCoder:scale:` in `TMSDAnimatedImage` with animated coder which conform to `TMSDProgressiveImageCoder` as well. Then call incremental update and incremental decode method to produce the image.
...
/**
 If the image which conforms to `TMSDAnimatedImage` protocol has more than one frame, set this value to `YES` will automatically
