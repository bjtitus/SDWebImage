...
typedef NSString * TMSDImageCoderOption NS_STRING_ENUM;
typedef NSDictionary<TMSDImageCoderOption, id> TMSDImageCoderOptions;
typedef NSMutableDictionary<TMSDImageCoderOption, id> TMSDImageCoderMutableOptions;
...
/**
 ...
 @note works for `TMSDImageCoder`.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderDecodeFirstFrameOnly;
...
/**
 ...
 @note works for `TMSDImageCoder`, `TMSDProgressiveImageCoder`, `TMSDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderDecodeScaleFactor;
...
/**
 ...
 @note works for `TMSDImageCoder`, `TMSDProgressiveImageCoder`, `TMSDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderDecodePreserveAspectRatio;
...
/**
 ...
 @note works for `TMSDImageCoder`, `TMSDProgressiveImageCoder`, `TMSDAnimatedImageCoder`.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderDecodeThumbnailPixelSize;
...
/**
 ...
 @note works for `TMSDImageCoder`.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderEncodeFirstFrameOnly;
/**
 ...
 @note works for `TMSDImageCoder`
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderEncodeCompressionQuality;
...
/**
 ...
 @note works for `TMSDImageCoder`
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderEncodeBackgroundColor;
...
/**
 ...
 @note works for `TMSDImageCoder`
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderEncodeMaxFileSize;
...
/**
 ...
 @note works for `TMSDImageCoder`
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderEncodeEmbedThumbnail;

/**
 A TMSDWebImageContext object which hold the original context options from top-level API. (TMSDWebImageContext)
 ...
 See `TMSDWebImageContext` for more detailed information.
 */
FOUNDATION_EXPORT TMSDImageCoderOption _Nonnull const TMSDImageCoderWebImageContext API_DEPRECATED("The coder component will be seperated from Core subspec in the future. Update your code to not rely on this context option.", macos(10.10, API_TO_BE_DEPRECATED), ios(8.0, API_TO_BE_DEPRECATED), tvos(9.0, API_TO_BE_DEPRECATED), watchos(2.0, API_TO_BE_DEPRECATED));
 ...
 */
@protocol TMSDImageCoder <NSObject>
...
/**
 ...
 @note This protocol may supports decode animated image frames. You can use `+[TMSDImageCoderHelper animatedImageWithFrames:]` to produce an animated image with frames.

 @param data The image data to be decoded
 @param options A dictionary containing any decoding options. Pass @{TMSDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for image. Pass @{TMSDImageCoderDecodeFirstFrameOnly: @(YES)} to decode the first frame only.
...
/**
 ...
 For custom coder which introduce new image format, you'd better define a new `TMSDImageFormat` using like this. If you're creating public coder plugin for new image format, also update `https://github.com/rs/TMSDWebImage/wiki/Coder-Plugin-List` to avoid same value been defined twice.
 * @code
 static const TMSDImageFormat TMSDImageFormatHEIF = 10;
 * @endcode
 
 @param format The image format
 @return YES if this coder can encode the image, NO otherwise
 */
- (BOOL)canEncodeToFormat:(TMSDImageFormat)format NS_SWIFT_NAME(canEncode(to:));

/**
 Encode the image to image data.
 @note This protocol may supports encode animated image frames. You can use `+[TMSDImageCoderHelper framesFromAnimatedImage:]` to assemble an animated image with frames.

 @param image The image to be encoded
 @param format The image format to encode, you should note `TMSDImageFormatUndefined` format is also  possible
 @param options A dictionary containing any encoding options. Pass @{TMSDImageCoderEncodeCompressionQuality: @(1)} to specify compression quality.
 @return The encoded image data
 */
- (nullable NSData *)encodedDataWithImage:(nullable UIImage *)image
                                   format:(TMSDImageFormat)format
                                  options:(nullable TMSDImageCoderOptions *)options;
...
/**
 ...
 @note Pay attention that these methods are not called from main queue.
 */
@protocol TMSDProgressiveImageCoder <TMSDImageCoder>
...
/**
 ...
 @param options A dictionary containing any progressive decoding options (instance-level). Pass @{TMSDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for progressive animated image (each frames should use the same scale).
 @return A new instance to do incremental decoding for the specify image format
 */
- (nonnull instancetype)initIncrementalWithOptions:(nullable TMSDImageCoderOptions *)options;
...
/**
 ...
 @note Due to the performance issue for progressive decoding and the integration for image view. This method may only return the first frame image even if the image data is animated image. If you want progressive animated image decoding, conform to `TMSDAnimatedImageCoder` protocol as well and use `animatedImageFrameAtIndex:` instead.

 @param options A dictionary containing any progressive decoding options. Pass @{TMSDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for progressive image
 @return The decoded image from current data
 */
- (nullable UIImage *)incrementalDecodedImageWithOptions:(nullable TMSDImageCoderOptions *)options;

@end

#pragma mark - Animated Image Provider
/**
 This is the animated image protocol to provide the basic function for animated image rendering. It's adopted by `TMSDAnimatedImage` and `TMSDAnimatedImageCoder`
 */
@protocol TMSDAnimatedImageProvider <NSObject>
...
/**
 This is the animated image coder protocol for custom animated image class like  `TMSDAnimatedImage`. Through it inherit from `TMSDImageCoder`. We currentlly only use the method `canDecodeFromData:` to detect the proper coder for specify animated image format.
 */
@protocol TMSDAnimatedImageCoder <TMSDImageCoder, TMSDAnimatedImageProvider>
...
/**
 ...
 After the instance created, we may call methods in `TMSDAnimatedImageProvider` to produce animated image frame.

 @param data The animated image data to be decode
 @param options A dictionary containing any animated decoding options (instance-level). Pass @{TMSDImageCoderDecodeScaleFactor: @(1.0)} to specify scale factor for animated image (each frames should use the same scale).
