...
@implementation TMSDImageAWebPCoder
...
+ (instancetype)sharedCoder {
    static TMSDImageAWebPCoder *coder;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        coder = [[TMSDImageAWebPCoder alloc] init];
    });
    return coder;
}
...
- (BOOL)canDecodeFromData:(nullable NSData *)data {
    switch ([NSData tmsd_imageFormatForImageData:data]) {
        case TMSDImageFormatWebP:
            ...
            return [self.class canDecodeFromFormat:TMSDImageFormatWebP];
        default:
            return NO;
    }
}
...
- (BOOL)canEncodeToFormat:(TMSDImageFormat)format {
    switch (format) {
        case TMSDImageFormatWebP:
            ...
            return [self.class canEncodeToFormat:TMSDImageFormatWebP];
        default:
            return NO;
    }
}
