...
static inline size_t TMSDByteAlign(size_t size, size_t alignment) {
    return ((size + (alignment - 1)) / alignment) * alignment;
}

#if TMSD_UIKIT
static inline UIImage *TMSDImageDecodeUIKit(UIImage *image) {
    ...
    if (@available(iOS 15, tvOS 15, *)) {
        ...
        if (decodedImage) {
            TMSDImageCopyAssociatedObject(image, decodedImage);
            decodedImage.tmsd_isDecoded = YES;
            return decodedImage;
        }
    }
    return nil;
...
static inline UIImage *TMSDImageDecodeAndScaleDownUIKit(UIImage *image, CGSize destResolution) {
...
static inline BOOL TMSDImageSupportsHardwareHEVCDecoder(void) {
    static dispatch_once_t onceToken;
    static BOOL supportsHardware = NO;
    dispatch_once(&onceToken, ^{
        SEL DeviceInfoSelector = TMSD_SEL_SPI(deviceInfoForKey:);
...
static TMSDImageCoderDecodeSolution kDefaultDecodeSolution = TMSDImageCoderDecodeSolutionAutomatic;
...
/*
 * Defines the maximum size in MB of the decoded image when the flag `TMSDWebImageScaleDownLargeImages` is set
...
@implementation TMSDImageCoderHelper

+ (UIImage *)animatedImageWithFrames:(NSArray<TMSDImageFrame *> *)frames {
    ...
    [frames enumerateObjectsUsingBlock:^(TMSDImageFrame * _Nonnull frame, NSUInteger idx, BOOL * _Nonnull stop) {
    ...
    CFStringRef imageUTType = [NSData tmsd_UTTypeFromImageFormat:TMSDImageFormatGIF];
    ...
    TMSDAnimatedImageRep *imageRep = [[TMSDAnimatedImageRep alloc] initWithData:imageData];
...
+ (NSArray<TMSDImageFrame *> *)framesFromAnimatedImage:(UIImage *)animatedImage {
    if (!animatedImage) {
        return nil;
    }
    ...
    [animatedImages enumerateObjectsUsingBlock:^(UIImage * _Nonnull image, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        if ([image isEqual:previousImage]) {
            ...
            TMSDImageFrame *frame = [TMSDImageFrame frameWithImage:previousImage duration:avgDuration * repeatCount];
    ...
    for (size_t i = 0; i < frameCount; i++) {
        @autoreleasepool {
            ...
            TMSDImageFrame *frame = [TMSDImageFrame frameWithImage:frameImage duration:frameDuration];
            [frames addObject:frame];
        }
    }
#endif
...
+ (CGImageRef)CGImageCreateDecoded:(CGImageRef)cgImage orientation:(CGImagePropertyOrientation)orientation {
    ...
    // Apply transform
    CGAffineTransform transform = TMSDCGContextTransformFromOrientation(orientation, CGSizeMake(newWidth, newHeight));
...
+ (CGImageRef)CGImageCreateScaled:(CGImageRef)cgImage size:(CGSize)size {
    ...
    output_buffer.rowBytes = TMSDByteAlign(output_buffer.width * 4, 64);
...
#if TMSD_UIKIT
    TMSDImageCoderDecodeSolution decodeSolution = self.defaultDecodeSolution;
    if (decodeSolution == TMSDImageCoderDecodeSolutionAutomatic) {
        ...
        if ((format == TMSDImageFormatHEIC || format == TMSDImageFormatHEIF) && TMSDImageSupportsHardwareHEVCDecoder()) {
            decodedImage = TMSDImageDecodeUIKit(image);
        } else if (format == TMSDImageFormatJPEG) {
            decodedImage = TMSDImageDecodeUIKit(image);
        }
    } else if (decodeSolution == TMSDImageCoderDecodeSolutionUIKit) {
...
+ (UIImage *)decodedAndScaledDownImageWithImage:(UIImage *)image limitBytes:(NSUInteger)bytes {
    ...
    if (decodeSolution == TMSDImageCoderDecodeSolutionAutomatic) {
        ...
        TMSDImageFormat format = image.tmsd_imageFormat;
        if ((format == TMSDImageFormatHEIC || format == TMSDImageFormatHEIF) && TMSDImageSupportsHardwareHEVCDecoder()) {
            decodedImage = TMSDImageDecodeAndScaleDownUIKit(image, destResolution);
        } else if (format == TMSDImageFormatJPEG) {
            decodedImage = TMSDImageDecodeAndScaleDownUIKit(image, destResolution);
        }
    } else if (decodeSolution == TMSDImageCoderDecodeSolutionUIKit) {
    ...
    // on iOS7, do not forget to call [[TMSDImageCache sharedImageCache] clearMemory];
    @autoreleasepool {
        // device color space
...
+ (TMSDImageCoderDecodeSolution)defaultDecodeSolution {
    return kDefaultDecodeSolution;
}

+ (void)setDefaultDecodeSolution:(TMSDImageCoderDecodeSolution)defaultDecodeSolution {
...
static inline CGAffineTransform TMSDCGContextTransformFromOrientation(CGImagePropertyOrientation orientation, CGSize size) {
