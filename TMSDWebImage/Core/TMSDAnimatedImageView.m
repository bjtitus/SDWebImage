...
@interface TMSDAnimatedImageView () {
    ...
    TMSDAnimatedImagePlaybackMode _playbackMode;
}

@property (nonatomic, strong, readwrite) TMSDAnimatedImagePlayer *player;
...
@implementation TMSDAnimatedImageView
#if TMSD_UIKIT
...
+ (instancetype)imageViewWithImage:(NSImage *)image
{
    ...
    TMSDAnimatedImageView *imageView = [[TMSDAnimatedImageView alloc] initWithFrame:frame];
...
- (void)setImage:(UIImage *)image
{
    ...
    if ([image.class conformsToProtocol:@protocol(TMSDAnimatedImage)]) {
        if (!self.player) {
            id<TMSDAnimatedImageProvider> provider;
            // Check progressive loading
            if (self.isProgressive) {
                provider = [self progressiveAnimatedCoderForImage:image];
            } else {
                provider = (id<TMSDAnimatedImage>)image;
            }
            // Create animated player
            self.player = [TMSDAnimatedImagePlayer playerWithProvider:provider];
        } else {
            // Update Frame Count
            self.player.totalFrameCount = [(id<TMSDAnimatedImage>)image animatedImageFrameCount];
        }
...
- (void)setPlaybackMode:(TMSDAnimatedImagePlaybackMode)playbackMode {
    _playbackMode = playbackMode;
    self.player.playbackMode = playbackMode;
}

- (TMSDAnimatedImagePlaybackMode)playbackMode {
    if (!_initFinished) {
        return TMSDAnimatedImagePlaybackModeNormal; // Default mode is normal
    }
...
- (void)checkPlay
{
    // Only handle for TMSDAnimatedImage, leave UIAnimatedImage or animationImages for super implementation control
...
- (void)updateIsProgressiveWithImage:(UIImage *)image
{
    ...
    id<TMSDAnimatedImageCoder> currentAnimatedCoder = [self progressiveAnimatedCoderForImage:image];
    if (currentAnimatedCoder) {
        ...
        if (!previousImage) {
            ...
            id<TMSDAnimatedImageCoder> previousAnimatedCoder = [self progressiveAnimatedCoderForImage:previousImage];
...
- (id<TMSDAnimatedImageCoder, TMSDProgressiveImageCoder>)progressiveAnimatedCoderForImage:(UIImage *)image
{
    if ([image.class conformsToProtocol:@protocol(TMSDAnimatedImage)] && image.tmsd_isIncremental && [image respondsToSelector:@selector(animatedCoder)]) {
        id<TMSDAnimatedImageCoder> animatedCoder = [(id<TMSDAnimatedImage>)image animatedCoder];
        if ([animatedCoder conformsToProtocol:@protocol(TMSDProgressiveImageCoder)]) {
            return (id<TMSDAnimatedImageCoder, TMSDProgressiveImageCoder>)animatedCoder;
        }
    }
    return nil;
}
...
- (NSView *)imageView {
    NSImageView *imageView = objc_getAssociatedObject(self, TMSD_SEL_SPI(imageView));
    if (!imageView) {
        // macOS 10.14
        imageView = objc_getAssociatedObject(self, TMSD_SEL_SPI(imageSubview));
    }
    return imageView;
}
