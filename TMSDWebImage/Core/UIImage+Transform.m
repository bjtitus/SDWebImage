...
static inline CGRect TMSDCGRectFitWithScaleMode(CGRect rect, CGSize size, TMSDImageScaleMode scaleMode) {
    rect = CGRectStandardize(rect);
    size.width = size.width < 0 ? -size.width : size.width;
    size.height = size.height < 0 ? -size.height : size.height;
    CGPoint center = CGPointMake(CGRectGetMidX(rect), CGRectGetMidY(rect));
    switch (scaleMode) {
        case TMSDImageScaleModeAspectFit:
        case TMSDImageScaleModeAspectFill: {
            if (rect.size.width < 0.01 || rect.size.height < 0.01 ||
                ...
                CGFloat scale;
                if (scaleMode == TMSDImageScaleModeAspectFit) {
...
static inline UIColor * TMSDGetColorFromGrayscale(Pixel_88 pixel, CGBitmapInfo bitmapInfo) {
...
static inline UIColor * TMSDGetColorFromRGBA(Pixel_8888 pixel, CGBitmapInfo bitmapInfo) {
    // Get alpha info, byteOrder info
    CGImageAlphaInfo alphaInfo = bitmapInfo & kCGBitmapAlphaInfoMask;
    CGBitmapInfo byteOrderInfo = bitmapInfo & kCGBitmapByteOrderMask;
...
static inline CGImageRef _Nullable TMSDCreateCGImageFromCIImage(CIImage * _Nonnull ciImage) {
...
@implementation UIImage (TMSDTransform)

- (void)tmsd_drawInRect:(CGRect)rect context:(CGContextRef)context scaleMode:(TMSDImageScaleMode)scaleMode clipsToBounds:(BOOL)clips {
    CGRect drawRect = TMSDCGRectFitWithScaleMode(rect, self.size, scaleMode);
...
- (nullable UIImage *)tmsd_resizedImageWithSize:(CGSize)size scaleMode:(TMSDImageScaleMode)scaleMode {
    ...
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
        [self tmsd_drawInRect:CGRectMake(0, 0, size.width, size.height) context:context scaleMode:scaleMode clipsToBounds:NO];
    }];
    return image;
}
...
- (nullable UIImage *)tmsd_roundedCornerImageWithRadius:(CGFloat)cornerRadius corners:(TMSDRectCorner)corners borderWidth:(CGFloat)borderWidth borderColor:(nullable UIColor *)borderColor {
    TMSDGraphicsImageRendererFormat *format = [[TMSDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    TMSDGraphicsImageRenderer *renderer = [[TMSDGraphicsImageRenderer alloc] initWithSize:self.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
#else
            NSBezierPath *path = [NSBezierPath tmsd_bezierPathWithRoundedRect:CGRectInset(rect, borderWidth, borderWidth) byRoundingCorners:corners cornerRadius:cornerRadius];
#endif
            [path closePath];
...
#else
            NSBezierPath *path = [NSBezierPath tmsd_bezierPathWithRoundedRect:strokeRect byRoundingCorners:corners cornerRadius:strokeRadius];
#endif
            [path closePath];
...
- (nullable UIImage *)tmsd_rotatedImageWithAngle:(CGFloat)angle fitSize:(BOOL)fitSize {
    ...
    TMSDGraphicsImageRendererFormat *format = [[TMSDGraphicsImageRendererFormat alloc] init];
    format.scale = self.scale;
    TMSDGraphicsImageRenderer *renderer = [[TMSDGraphicsImageRenderer alloc] initWithSize:newRect.size format:format];
    UIImage *image = [renderer imageWithActions:^(CGContextRef  _Nonnull context) {
...
- (nullable UIColor *)tmsd_colorAtPoint:(CGPoint)point {
...
#if TMSD_UIKIT || TMSD_MAC
    if (self.CIImage) {
        imageRef = TMSDCreateCGImageFromCIImage(self.CIImage);
    }
#endif
    if (!imageRef) {
    ...
    if (components == 2) {
        ...
        CGImageRelease(imageRef);
        // Convert to color
        return TMSDGetColorFromGrayscale(pixel, bitmapInfo);
        ...
        CGImageRelease(imageRef);
        // Convert to color
        return TMSDGetColorFromRGBA(pixel, bitmapInfo);
    } else {
...
- (nullable NSArray<UIColor *> *)tmsd_colorsWithRect:(CGRect)rect {
    ...
    for (size_t index = start; index < end; index += components) {
        ...
        if (components == 2) {
            ...
            color = TMSDGetColorFromGrayscale(pixel, bitmapInfo);
        } else {
            if (components == 3) {
                Pixel_8888 pixel = {pixels[index], pixels[index+1], pixels[index+2], 0};
                color = TMSDGetColorFromRGBA(pixel, bitmapInfo);
...
- (nullable UIImage *)tmsd_blurredImageWithRadius:(CGFloat)blurRadius {
    ...
    if (CGImageGetBitsPerPixel(imageRef) != 32 ||
        ...
        TMSDGraphicsBeginImageContextWithOptions(self.size, NO, self.scale);
        [self drawInRect:CGRectMake(0, 0, self.size.width, self.size.height)];
        imageRef = TMSDGraphicsGetImageFromCurrentImageContext().CGImage;
