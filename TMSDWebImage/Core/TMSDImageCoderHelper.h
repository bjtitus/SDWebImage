...
typedef NS_ENUM(NSUInteger, TMSDImageCoderDecodeSolution) {
    /// automatically choose the solution based on image format, hardware, OS version. This keep balance for compatibility and performance. Default after TMSDWebImage 5.13.0
    TMSDImageCoderDecodeSolutionAutomatic,
    /// always use CoreGraphics to draw on bitmap context and trigger decode. Best compatibility. Default before TMSDWebImage 5.13.0
    TMSDImageCoderDecodeSolutionCoreGraphics,
 ...
 */
+ (UIImage * _Nullable)animatedImageWithFrames:(NSArray<TMSDImageFrame *> * _Nullable)frames;
 ...
 */
+ (NSArray<TMSDImageFrame *> * _Nullable)framesFromAnimatedImage:(UIImage * _Nullable)animatedImage NS_SWIFT_NAME(frames(from:));
...
/**
 Control the default force decode solution. Available solutions  in `TMSDImageCoderDecodeSolution`.
 @note Defaults to `TMSDImageCoderDecodeSolutionAutomatic`, which prefers to use UIKit for JPEG/HEIF, and fallback on CoreGraphics. If you want control on your hand, set the other solution.
 */
@property (class, readwrite) TMSDImageCoderDecodeSolution defaultDecodeSolution;
