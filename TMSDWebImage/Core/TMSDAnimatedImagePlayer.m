...
#import <TMSDWebImage/TMSDDeviceHelper.h>
...
@property (nonatomic, strong) id<TMSDAnimatedImageProvider> animatedProvider;
...
@property (nonatomic, strong) TMSDDisplayLink *displayLink;

@end

@implementation TMSDAnimatedImagePlayer

- (instancetype)initWithProvider:(id<TMSDAnimatedImageProvider>)provider {
    self = [super init];
    if (self) {
...
+ (instancetype)playerWithProvider:(id<TMSDAnimatedImageProvider>)provider {
    TMSDAnimatedImagePlayer *player = [[TMSDAnimatedImagePlayer alloc] initWithProvider:provider];
    return player;
}
...
- (TMSDDisplayLink *)displayLink {
    if (!_displayLink) {
        _displayLink = [TMSDDisplayLink displayLinkWithTarget:self selector:@selector(displayDidRefresh:)];
...
- (void)setupCurrentFrame {
    ...
    if (self.playbackMode == TMSDAnimatedImagePlaybackModeReverse ||
               self.playbackMode == TMSDAnimatedImagePlaybackModeReversedBounce) {
...
- (void)stopPlaying {
    [_fetchQueue cancelAllOperations];
    // Using `_displayLink` here because when UIImageView dealloc, it may trigger `[self stopAnimating]`, we already release the display link in TMSDAnimatedImageView's dealloc method.
    [_displayLink stop];
...
- (void)displayDidRefresh:(TMSDDisplayLink *)displayLink {
    ...
    } else if (self.playbackMode == TMSDAnimatedImagePlaybackModeBounce ||
               self.playbackMode == TMSDAnimatedImagePlaybackModeReversedBounce) {
    ...
    if (!fetchFrame && !bufferFull && self.fetchQueue.operationCount == 0) {
        ...
        id<TMSDAnimatedImageProvider> animatedProvider = self.animatedProvider;
        @weakify(self);
...
- (void)calculateMaxBufferCount {
    ...
    if (self.maxBufferSize > 0) {
        ...
        NSUInteger total = [TMSDDeviceHelper totalMemory];
        NSUInteger free = [TMSDDeviceHelper freeMemory];
