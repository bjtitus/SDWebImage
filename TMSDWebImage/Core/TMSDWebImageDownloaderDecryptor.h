...
typedef NSData * _Nullable (^TMSDWebImageDownloaderDecryptorBlock)(NSData * _Nonnull data, NSURLResponse * _Nullable response);
...
*/
@protocol TMSDWebImageDownloaderDecryptor <NSObject>
...
/// @note If nil is returned, the image download will be marked as failed with error `TMSDWebImageErrorBadImageData`
...
*/
@interface TMSDWebImageDownloaderDecryptor : NSObject <TMSDWebImageDownloaderDecryptor>

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
- (nonnull instancetype)initWithBlock:(nonnull TMSDWebImageDownloaderDecryptorBlock)block;

/// Create the data decryptor with block
/// @param block A block to control decrypt logic
+ (nonnull instancetype)decryptorWithBlock:(nonnull TMSDWebImageDownloaderDecryptorBlock)block;

@end

/// Convenience way to create decryptor for common data encryption.
@interface TMSDWebImageDownloaderDecryptor (TMSDConveniences)

/// Base64 Encoded image data decryptor
@property (class, readonly, nonnull) TMSDWebImageDownloaderDecryptor *base64Decryptor;
