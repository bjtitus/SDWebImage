...
/// Image Cache Options
typedef NS_OPTIONS(NSUInteger, TMSDImageCacheOptions) {
    /**
     * By default, we do not query image data when the image is already cached in memory. This mask can force to query image data at the same time. However, this query is asynchronously unless you specify `TMSDImageCacheQueryMemoryDataSync`
     */
    TMSDImageCacheQueryMemoryData = 1 << 0,
    /**
     * By default, when you only specify `TMSDImageCacheQueryMemoryData`, we query the memory image data asynchronously. Combined this mask as well to query the memory image data synchronously.
     */
    TMSDImageCacheQueryMemoryDataSync = 1 << 1,
    /**
     ...
     */
    TMSDImageCacheQueryDiskDataSync = 1 << 2,
    /**
     * By default, images are decoded respecting their original size. On iOS, this flag will scale down the
     * images to a size compatible with the constrained memory of devices.
     */
    TMSDImageCacheScaleDownLargeImages = 1 << 3,
    /**
     * By default, we will decode the image in the background during cache query and download from the network. This can help to improve performance because when rendering image on the screen, it need to be firstly decoded. But this happen on the main queue by Core Animation.
     * However, this process may increase the memory usage as well. If you are experiencing a issue due to excessive memory consumption, This flag can prevent decode the image.
     */
    TMSDImageCacheAvoidDecodeImage = 1 << 4,
    /**
     * By default, we decode the animated image. This flag can force decode the first frame only and produce the static image.
     */
    TMSDImageCacheDecodeFirstFrameOnly = 1 << 5,
    /**
     * By default, for `TMSDAnimatedImage`, we decode the animated image frame during rendering to reduce memory usage. This flag actually trigger `preloadAllAnimatedImageFrames = YES` after image load from disk cache
     */
    TMSDImageCachePreloadAllFrames = 1 << 6,
    /**
     ...
     * Using this option, can ensure we always produce image with your provided class. If failed, an error with code `TMSDWebImageErrorBadImageData` will be used.
     * Note this options is not compatible with `TMSDImageCacheDecodeFirstFrameOnly`, which always produce a UIImage/NSImage.
     */
    TMSDImageCacheMatchAnimatedImageClass = 1 << 7,
};

/**
 *  A token associated with each cache query. Can be used to cancel a cache query
 */
@interface TMSDImageCacheToken : NSObject <TMSDWebImageOperation>
...
/**
 * TMSDImageCache maintains a memory cache and a disk cache. Disk cache write operations are performed
 * asynchronous so it doesn’t add unnecessary latency to the UI.
 */
@interface TMSDImageCache : NSObject
 ...
 */
@property (nonatomic, copy, nonnull, readonly) TMSDImageCacheConfig *config;

/**
 * The memory cache implementation object used for current image cache.
 * By default we use `TMSDMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `TMSDImageCacheConfig.memoryCacheClass` property.
 */
@property (nonatomic, strong, readonly, nonnull) id<TMSDMemoryCache> memoryCache;

/**
 * The disk cache implementation object used for current image cache.
 * By default we use `TMSDMemoryCache` class, you can also use this to call your own implementation class method.
 * @note To customize this class, check `TMSDImageCacheConfig.diskCacheClass` property.
 ...
 */
@property (nonatomic, strong, readonly, nonnull) id<TMSDDiskCache> diskCache;
 ...
 */
@property (nonatomic, copy, nullable) TMSDImageCacheAdditionalCachePathBlock additionalCachePathBlock;
 ...
 */
@property (nonatomic, class, readonly, nonnull) TMSDImageCache *sharedImageCache;

/**
 * Control the default disk cache directory. This will effect all the TMSDImageCache instance created after modification, even for shared image cache.
 ...
 * @note If you pass nil, the value will be reset to `~/Library/Caches/com.hackemist.TMSDImageCache`.
 * @note We still preserve the `namespace` arg, which means, if you change this property into `/path/to/use`,  the `TMSDImageCache.sharedImageCache.diskCachePath` should be `/path/to/use/default` because shared image cache use `default` as namespace.
 * Defaults to nil.
 */
 ...
 * The final disk cache directory should looks like ($directory/$namespace). And the default config of shared cache, should result in (~/Library/Caches/com.hackemist.TMSDImageCache/default/)
 *
...
- (nonnull instancetype)initWithNamespace:(nonnull NSString *)ns
                       diskCacheDirectory:(nullable NSString *)directory
                                   config:(nullable TMSDImageCacheConfig *)config NS_DESIGNATED_INITIALIZER;
 ...
 * @param completionBlock A block executed after the operation is finished
 * @note If no image data is provided and encode to disk, we will try to detect the image format (using either `tmsd_imageFormat` or `TMSDAnimatedImage` protocol method) and animation status, to choose the best matched format, including GIF, JPEG or PNG.
 */
- (void)storeImage:(nullable UIImage *)image
            forKey:(nullable NSString *)key
            toDisk:(BOOL)toDisk
 ...
 *  @note the completion block will be always executed on the main queue
 */
- (void)diskImageDataQueryForKey:(nullable NSString *)key completion:(nullable TMSDImageCacheQueryDataCompletionBlock)completionBlock;

/**
 * Asynchronously queries the cache with operation and call the completion when done.
 *
 * @param key       The unique key used to store the wanted image. If you want transformed or thumbnail image, calculate the key with `TMSDTransformedKeyForKey`, `TMSDThumbnailedKeyForKey`, or generate the cache key from url with `cacheKeyForURL:context:`.
 * @param doneBlock The completion block. Will not get called if the operation is cancelled
 *
 * @return a TMSDImageCacheToken instance containing the cache operation, will callback immediately when cancelled
 */
- (nullable TMSDImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key done:(nullable TMSDImageCacheQueryCompletionBlock)doneBlock;
 ...
 * @return a TMSDImageCacheToken instance containing the cache operation, will callback immediately when cancelled
 */
- (nullable TMSDImageCacheToken *)queryCacheOperationForKey:(nullable NSString *)key options:(TMSDImageCacheOptions)options done:(nullable TMSDImageCacheQueryCompletionBlock)doneBlock;
 ...
 * @param doneBlock The completion block. Will not get called if the operation is cancelled
 *
 * @return a TMSDImageCacheToken instance containing the cache operation, will callback immediately when cancellederation, will callback immediately when cancelled
 */
...
/**
 * TMSDImageCache is the built-in image cache implementation for web image manager. It adopts `TMSDImageCache` protocol to provide the function for web image manager to use for image loading process.
 */
@interface TMSDImageCache (TMSDTMSDImageCache) <TMSDImageCache>
