...
typedef NS_ENUM(NSInteger, TMSDWebImageDownloaderExecutionOrder) {
    /**
     * Default value. All download operations will execute in queue style (first-in-first-out).
     */
    TMSDWebImageDownloaderFIFOExecutionOrder,
    
    /**
     * All download operations will execute in stack style (last-in-first-out).
     */
    TMSDWebImageDownloaderLIFOExecutionOrder
};
 ...
 */
@interface TMSDWebImageDownloaderConfig : NSObject <NSCopying>

/**
 Gets the default downloader config used for shared instance or initialization when it does not provide any downloader config. Such as `TMSDWebImageDownloader.sharedDownloader`.
 ...
 */
@property (nonatomic, class, readonly, nonnull) TMSDWebImageDownloaderConfig *defaultDownloaderConfig;
...
/**
 * Gets/Sets a subclass of `TMSDWebImageDownloaderOperation` as the default
 * `NSOperation` to be used each time TMSDWebImage constructs a request
 * operation to download an image.
 * Defaults to nil.
 * @note Passing `NSOperation<TMSDWebImageDownloaderOperation>` to set as default. Passing `nil` will revert to `TMSDWebImageDownloaderOperation`.
 */
@property (nonatomic, assign, nullable) Class operationClass;

/**
 * Changes download operations execution order.
 * Defaults to `TMSDWebImageDownloaderFIFOExecutionOrder`.
 */
@property (nonatomic, assign) TMSDWebImageDownloaderExecutionOrder executionOrder;
 ...
 * For example, if we config [200, 400) but server response is 503, the download will fail with error code `TMSDWebImageErrorInvalidDownloadStatusCode`.
 ...
 * For example, if we config ["image/png"] but server response is "application/json", the download will fail with error code `TMSDWebImageErrorInvalidDownloadContentType`.
