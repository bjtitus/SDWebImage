...
static TMSDImageCacheConfig *_defaultCacheConfig;
static const NSInteger kDefaultCacheMaxDiskAge = 60 * 60 * 24 * 7; // 1 week

@implementation TMSDImageCacheConfig

+ (TMSDImageCacheConfig *)defaultCacheConfig {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _defaultCacheConfig = [TMSDImageCacheConfig new];
    });
...
- (instancetype)init {
    if (self = [super init]) {
        ...
        _maxDiskSize = 0;
        _diskCacheExpireType = TMSDImageCacheConfigExpireTypeModificationDate;
        _memoryCacheClass = [TMSDMemoryCache class];
        _diskCacheClass = [TMSDDiskCache class];
    }
    return self;
}

- (id)copyWithZone:(NSZone *)zone {
    TMSDImageCacheConfig *config = [[[self class] allocWithZone:zone] init];
