...
/**
 ...
 For others (macOS/watchOS or iOS/tvOS 10-), these method use the `TMSDImageGraphics.h` to implements the same behavior (but without dynamic bitmap support)
*/

typedef void (^TMSDGraphicsImageDrawingActions)(CGContextRef _Nonnull context);
typedef NS_ENUM(NSInteger, TMSDGraphicsImageRendererFormatRange) {
    TMSDGraphicsImageRendererFormatRangeUnspecified = -1,
    TMSDGraphicsImageRendererFormatRangeAutomatic = 0,
    TMSDGraphicsImageRendererFormatRangeExtended,
    TMSDGraphicsImageRendererFormatRangeStandard
};

/// A set of drawing attributes that represent the configuration of an image renderer context.
@interface TMSDGraphicsImageRendererFormat : NSObject
...
@property (nonatomic) TMSDGraphicsImageRendererFormatRange preferredRange;
...
@interface TMSDGraphicsImageRenderer : NSObject
...
/// @param size The size of images output from the renderer, specified in points.
/// @param format A TMSDGraphicsImageRendererFormat object that encapsulates the format used to create the renderer context.
/// @return An initialized image renderer.
- (nonnull instancetype)initWithSize:(CGSize)size format:(nonnull TMSDGraphicsImageRendererFormat *)format;

/// Creates an image by following a set of drawing instructions.
/// @param actions A TMSDGraphicsImageDrawingActions block that, when invoked by the renderer, executes a set of drawing instructions to create the output image.
...
- (nonnull UIImage *)imageWithActions:(nonnull NS_NOESCAPE TMSDGraphicsImageDrawingActions)actions;
