...
static NSString * const TMSDDiskCacheExtendedAttributeName = @"com.hackemist.TMSDDiskCache";

@interface TMSDDiskCache ()
...
#pragma mark - TMSDcachePathForKeyDiskCache Protocol
- (instancetype)initWithCachePath:(NSString *)cachePath config:(nonnull TMSDImageCacheConfig *)config {
    if (self = [super init]) {
...
- (BOOL)containsDataForKey:(NSString *)key {
    ...
    // fallback because of https://github.com/rs/TMSDWebImage/pull/976 that added the extension to the disk file name
...
- (NSData *)extendedDataForKey:(NSString *)key {
    ...
    NSData *extendedData = [TMSDFileAttributeHelper extendedAttribute:TMSDDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
...
- (void)setExtendedData:(NSData *)extendedData forKey:(NSString *)key {
    ...
    if (!extendedData) {
        // Remove
        [TMSDFileAttributeHelper removeExtendedAttribute:TMSDDiskCacheExtendedAttributeName atPath:cachePathForKey traverseLink:NO error:nil];
    } else {
        // Override
        [TMSDFileAttributeHelper setExtendedAttribute:TMSDDiskCacheExtendedAttributeName value:extendedData atPath:cachePathForKey traverseLink:NO overwrite:YES error:nil];
    }
}
...
- (void)removeExpiredData {
    ...
    switch (self.config.diskCacheExpireType) {
        case TMSDImageCacheConfigExpireTypeAccessDate:
            cacheContentDateKey = NSURLContentAccessDateKey;
            break;
        case TMSDImageCacheConfigExpireTypeModificationDate:
            cacheContentDateKey = NSURLContentModificationDateKey;
            break;
        case TMSDImageCacheConfigExpireTypeCreationDate:
            cacheContentDateKey = NSURLCreationDateKey;
            break;
        case TMSDImageCacheConfigExpireTypeChangeDate:
...
- (nullable NSString *)cachePathForKey:(nullable NSString *)key inPath:(nonnull NSString *)path {
    NSString *filename = TMSDDiskCacheFileNameForKey(key);
...
#define TMSD_MAX_FILE_EXTENSION_LENGTH (NAME_MAX - CC_MD5_DIGEST_LENGTH * 2 - 1)

#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
static inline NSString * _Nonnull TMSDDiskCacheFileNameForKey(NSString * _Nullable key) {
    ...
    if (ext.length > TMSD_MAX_FILE_EXTENSION_LENGTH) {
        ext = nil;
    }
