 ...
 */
FOUNDATION_EXPORT const int64_t TMSDWebImageProgressUnitCountUnknown; /* 1LL */

typedef void(^TMSDSetImageBlock)(UIImage * _Nullable image, NSData * _Nullable imageData, TMSDImageCacheType cacheType, NSURL * _Nullable imageURL);

/**
 Integrates TMSDWebImage async downloading and caching of remote images with UIView subclass.
 */
@interface UIView (TMSDWebCache)
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSURL *tmsd_imageURL;
 ...
 * See more about this in `TMSDWebImageContextSetImageOperationKey`.
 ...
 */
@property (nonatomic, strong, readonly, nullable) NSString *tmsd_latestOperationKey;
 ...
 * The `totalUnitCount` and `completedUnitCount` will be reset to 0 after a new image loading start (change from current queue). And they will be set to `TMSDWebImageProgressUnitCountUnknown` if the progressBlock not been called but the image loading success to mark the progress finished (change from main queue).
 ...
 */
@property (nonatomic, strong, null_resettable) NSProgress *tmsd_imageProgress;
 ...
 *
 *   The fifth parameter normally is always YES. However, if you provide TMSDWebImageAvoidAutoSetImage with TMSDWebImageProgressiveLoad options to enable progressive downloading and set the image yourself. This block is thus called repeatedly with a partial image. When image is fully downloaded, the
 *   block is called a last time with the full image and the last parameter set to YES.
 *
 *   The last parameter is the original image URL
 *  @return The returned operation for cancelling cache and download operation, typically type is `TMSDWebImageCombinedOperation`
 */
...
/**
 The image transition when image load finished. See `TMSDWebImageTransition`.
 If you specify nil, do not do transition. Defaults to nil.
 */
@property (nonatomic, strong, nullable) TMSDWebImageTransition *tmsd_imageTransition;
 ...
 */
@property (nonatomic, strong, nullable) id<TMSDWebImageIndicator> tmsd_imageIndicator;
