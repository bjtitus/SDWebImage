...
#import <TMSDWebImage/MasterViewController.h>
...
@property (nonatomic, strong) TMSDAnimatedImageView *customImageView;
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        _customImageView = [[TMSDAnimatedImageView alloc] initWithFrame:CGRectMake(20.0, 2.0, 60.0, 40.0)];
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    ...
    if (self) {
        ...
        [TMSDWebImageDownloader sharedDownloader].config.username = @"httpwatch";
        [TMSDWebImageDownloader sharedDownloader].config.password = @"httpwatch01";
        [[TMSDWebImageDownloader sharedDownloader] setValue:@"TMSDWebImage Demo" forHTTPHeaderField:@"AppName"];
        [TMSDWebImageDownloader sharedDownloader].config.executionOrder = TMSDWebImageDownloaderLIFOExecutionOrder;
...
- (void)flushCache {
    [TMSDWebImageManager.sharedManager.imageCache clearWithCacheType:TMSDImageCacheTypeAll completion:nil];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (cell == nil) {
        ...
        cell.customImageView.tmsd_imageTransition = TMSDWebImageTransition.fadeTransition;
        cell.customImageView.tmsd_imageIndicator = TMSDWebImageActivityIndicator.grayIndicator;
    }
    
    cell.customTextLabel.text = [NSString stringWithFormat:@"Image #%ld", (long)indexPath.row];
    __weak TMSDAnimatedImageView *imageView = cell.customImageView;
    [imageView tmsd_setImageWithURL:[NSURL URLWithString:self.objects[indexPath.row]]
                 placeholderImage:placeholderImage
                          options:0
                          context:@{TMSDWebImageContextImageThumbnailPixelSize : @(CGSizeMake(180, 120))}
                         progress:nil
                        completed:^(UIImage * _Nullable image, NSError * _Nullable error, TMSDImageCacheType cacheType, NSURL * _Nullable imageURL) {
        TMSDWebImageCombinedOperation *operation = [imageView tmsd_imageLoadOperationForKey:imageView.tmsd_latestOperationKey];
        TMSDWebImageDownloadToken *token = operation.loaderOperation;
